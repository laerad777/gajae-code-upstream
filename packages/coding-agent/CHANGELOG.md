...
- Usage report rendering now sanitizes provider-supplied account metadata before display. Labels drawn from untrusted usage responses are tab-expanded, stripped of control characters, and width-truncated in both the interactive usage view and the `/usage` slash command, so a hostile usage response cannot inject newlines or escape sequences into TUI output. Reports without account metadata are also numbered by position instead of collapsing onto a single shared label.
