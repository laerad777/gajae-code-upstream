...
- Kiro HTTP failures no longer interpolate the upstream response body into the thrown error. AWS error envelopes carry account ids, profile ARNs, and request ids, and the message reaches `AssistantMessage.errorMessage`, which is persisted to session transcripts and logs; only the status code is surfaced now. The `__type` fault discriminator is deliberately dropped as well: it is attacker-controlled text on a failure path, so it is not read and the response body is never consumed.
...
- Kiro Builder ID login no longer persists a synthetic profile ARN. The hardcoded `KIRO_BUILDER_ID_PROFILE_ARN` constant is removed: writing an unconfirmed value into the authoritative credential slot meant a stale constant would 403 forever instead of falling back to resolution. An empty slot now means "unresolved" and the real ARN is derived on first use.
- Kiro profile-ARN resolution follows one trust invariant across all four consumers (streaming, model discovery, usage, and login): a stored `kiroProfileArn` is server-confirmed and authoritative, and resolution happens only when the slot is empty. Model discovery previously discarded the credential ARN and the usage probe resolved unconditionally, so the same fact had three different owners.
...
- Kiro's device-registration table (`kiro_kv`) is now declared by the credential store's schema initialization instead of being created ad hoc on first write, and the read path probes for the table before querying it. A missing table is treated as an expected cold start while an unreadable database is reported distinctly, so a read-only or damaged database no longer drives unbounded client re-registration against AWS SSO-OIDC.
