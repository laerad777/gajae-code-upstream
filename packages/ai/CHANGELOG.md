...
- Kiro reasoning requests now follow each discovered model's `additionalModelRequestFieldsSchema`: GPT-5.6 sends only `reasoning`, while adaptive Claude models send `thinking` plus `output_config`. Kiro previously sent both `thinking` and `reasoning` to every reasoning model, causing GPT-5.6 Sol/Terra/Luna requests and delegated tasks to fail before token generation with HTTP 400.
- Usage reports no longer expose an account-identity discriminator to remote callers. The auth broker and auth gateway strip `metadata.accountIdentity` from every trimmed usage report, and the Kiro usage reporter derives a short one-way fingerprint from the opaque `userId` instead of surfacing the raw value, falling back to the credential-store row id when no server identity exists.
- Usage report cache keys are now structurally encoded. Provider, base URL, and account identity were previously joined with bare `:` separators, so a base URL or identity containing a colon could collide with a different account's key and serve one account's usage totals for another. Keys and the shared-refresh grouping key now use JSON encoding, and prefix invalidation matches the encoded provider segment.
- Concurrent OAuth refreshes no longer clobber each other. Credential replacement now performs a compare-and-swap against the expected stored row, matches by credential-store row id when one is known, and requires a real account identifier before falling back to identity matching, so two refreshes racing on the same provider cannot persist a stale token over a newer one. Per-credential retry backoff is also cleared once a credential refreshes successfully.
- Auth broker snapshot wire schemas now reject malformed credential ids and duplicate rows: ids must be positive safe integers, and a snapshot carrying two entries with the same id is rejected instead of silently shadowing one credential.
- Usage report rendering now sanitizes provider-supplied metadata. Account labels drawn from untrusted usage responses are tab-expanded, stripped of control characters, and width-truncated before reaching the TUI or the `/usage` slash command, and rows without account metadata are numbered by position so multiple identical reports no longer collapse onto the same label.
...
- Kiro HTTP and protocol failures no longer persist upstream details in session transcripts. Non-2xx responses are reduced to the status code without consuming the body; AWS event-stream exception payloads and HTTP-200 Builder ID or social polling errors now use fixed summaries instead of forwarding account ids, profile ARNs, request ids, or attacker-controlled text.
- Kiro OAuth bearer requests are pinned to the official runtime endpoint, so a custom or overridden `kiro-streaming` model URL cannot receive stored OAuth credentials.
- Shared OAuth refresh now persists a successful rotating token inside the single-flight operation even if every caller stops waiting, keeps other waiters alive when one caller cancels, and aborts the real Kiro token request at the AuthStorage deadline instead of abandoning a still-running fetch.
